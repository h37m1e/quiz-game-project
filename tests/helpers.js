...
async function resetDb() {
  ...
  await prisma.$executeRawUnsafe("DELETE FROM _KeywordToQuestion");
  await prisma.question.deleteMany();
  await prisma.keyword.deleteMany();
  await prisma.user.deleteMany();
}

async function registerAndLogin(email = "ab@test.io", name = "A") {
...
async function createQuestion(token, overrides = {}) {
  const res = await request(app).post("/api/questions")
    ...
    .send({ title: "T",  answer: "C", subject: "S", ...overrides });
  return res.body;
}
